...
export default function AdminDashboard() {
  ...
  const totalCollDigital = reports.reduce((a, r) => a + +r.card_upi_total, 0);
  ...
  const statCards = [
    ...
    { label: 'UPI / Card',      value: totalCollDigital, color: '#6366f1', icon: '📱' },
  ...
  const exportToExcel = async (mode) => {
    ...
    if (mode === 'report') {
      ...
      data.push({ 'Date': 'GRAND TOTAL', 'Cash': totalCash, 'UPI / Card': totalCollDigital, 'Credit Note': totalCN, 'Sodexo': totalSod, 'Cheques': totalChq, 'Expenses': totalExp, 'System Total': totalSys, 'Difference': diff, 'Grand Total': totalColl });
              ...
              {printMode === 'report' ? (
                reports.map((r, i) => (
                  <tr key={i}>
                    ...
                    <td style={{ color: '#818cf8', fontWeight: 600 }}>{fmt(r.card_upi_total)}</td>
